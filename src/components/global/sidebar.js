    ...
    <div className="nav">
...

        {
            type === USER_TYPE.AGENT ? (
                <React.Fragment>
                    <NavLink to="/agent/properties" isActive={(match, location) => {
                        const { pathname } = location;
                        return pathname === '/agent/properties' || pathname.includes('/agent/property-details');
                    }}>
                        My Properties
...

                    {
                        (userDetail?.agent?.agentType === AGENT_TYPE.AGENT || userDetail?.agentAccessLevels?.find((level) => level.accessLevel === AGENT_USER_ACCESS_TYPE_VALUE.ADD_PROPERTY)) && (
                            <NavLink to="/agent/add-property" isActive={(match, location) => {
                                const { pathname } = location;
                                return pathname === '/agent/add-property' || pathname.includes('/agent/edit-property');
                            }}>
                                Add Property
                ...
                <React.Fragment>
                    <NavLink to={`/customer/wishlist`} isActive={(match, location) => {
                        const { pathname } = location;
                        return pathname === '/customer/wishlist' || pathname.includes('/customer/property-details');
                    }}>
                        My Wishlist
                        <i className="fa-solid fa-list" />
                    </NavLink>
