...
const GoogleMapsSearch = () => {
  ...
  const searchPropertiesInPolygon = useCallback(
    async (polygon) => {
      try {
        ...
        const response = await HomepageService.searchByPolygon({
          coordinates: coordinates, // Correctly formatted for a POLYGON
        });
  ...
  useEffect(() => {
    ...
    if (inputElement && inputElement instanceof HTMLInputElement) {
      ...
      console.error("searchInputRef is not attached to a valid HTMLInputElement");
    }
  }, [isLoaded]); // Depend on the isLoaded state to re-run when Google script is loaded
      ...
      <div>
        ...
        >
          ...
          {polygons.map((polygon, index) => (
            <Polygon
              key={index}
              paths={polygon.getPath().getArray().map((latLng) => ({
                lat: latLng.lat(),
                lng: latLng.lng(),
              }))}
              options={{ fillColor: "#FF0000" }}
